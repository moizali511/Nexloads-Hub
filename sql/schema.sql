...
-- Idempotent add-on columns (safe to run again on a database that
-- already has the base employees table from an earlier version of
-- this file — ALTER .. ADD COLUMN IF NOT EXISTS never touches a
...
-- Idempotent add-ons: messages can now also come FROM an employee (not
-- just admin -> employee), and can carry one attached image (a deal
-- confirmation screenshot, a payment receipt, etc — stored as a base64
-- data URL, kept small by the frontend before it's sent).
alter table messages add column if not exists sender_employee_id uuid references employees(id) on delete cascade;
alter table messages add column if not exists image_data text;
alter table messages alter column sender_admin_id drop not null;
...
begin
  ...
  update employees set last_login_at = now(), last_seen_at = now() where id = emp.id;
...
-- NEW: called every ~45s from the frontend while someone is actively on
-- a dashboard, so admin can see a live "online now" status per employee.
create or replace function heartbeat(p_employee_id uuid)
returns json
language plpgsql
security definer
as $$
begin
  update employees set last_seen_at = now() where id = p_employee_id and active = true;
  if not found then
...
-- NEW: any active employee (not just admin) can change their own password,
-- as long as they correctly supply their current one. Same one-way bcrypt
-- hashing as everywhere else — the actual password is never stored or
-- shown anywhere, including to admin.
create or replace function change_my_password(
  p_employee_id uuid, p_current_password text, p_new_password text
)
returns json
language plpgsql
security definer
...
declare
  emp employees;
begin
  select * into emp from employees where id = p_employee_id and active = true;
...
-- NEW: admin can force-set a new password for any employee without
-- knowing their current one — this is how admin keeps guaranteed access
-- to every account (e.g. to open someone's dashboard, or hand them a
-- fresh login if they're locked out / leaving) without the system ever
-- storing anyone's actual password in a readable form.
create or replace function admin_reset_employee_password(
  p_admin_id uuid, p_employee_id uuid, p_new_password text
)
returns json
language plpgsql
security definer
...
-- Old 3-parameter version is explicitly dropped before the 4-parameter
-- replacement is created, so Postgres never ends up with two versions
-- of admin_send_message at once (that ambiguity is exactly what broke
-- "add employee" before — not repeating it here).
drop function if exists admin_send_message(uuid, uuid, text);

create or replace function admin_send_message(
  p_admin_id uuid, p_recipient_employee_id uuid, p_body text, p_image_data text default null
)
returns json
language plpgsql
security definer
...
begin
  ...
  if trim(coalesce(p_body,'')) = '' and p_image_data is null then
    return json_build_object('success', false, 'error', 'empty_message');
  end if;
  ...
  insert into messages (sender_admin_id, recipient_employee_id, body, image_data)
    values (p_admin_id, p_recipient_employee_id, coalesce(trim(p_body), ''), p_image_data)
...
-- NEW: an employee reporting to admin — a closed deal, a payment
-- screenshot, a question, anything — with an optional attached image.
-- Always goes to the shared admin inbox (there's no "pick one admin"
-- concept here, same as broadcasts going to all employees).
create or replace function employee_send_message(
  p_employee_id uuid, p_body text, p_image_data text default null
)
returns json
language plpgsql
security definer
...
begin
  ...
  insert into messages (sender_employee_id, recipient_employee_id, body, image_data)
    values (p_employee_id, null, coalesce(trim(p_body), ''), p_image_data)
    returning * into new_msg;
...
-- Now also surfaces employee-sent messages (not just admin broadcasts/
-- direct messages), any attached image, and who has read each one.
...
begin
  ...
  return json_build_object('success', true, 'messages', (
    ...
    from (
      select
        msg.id, msg.body, msg.created_at, msg.recipient_employee_id, msg.image_data,
        (msg.sender_employee_id is not null) as is_from_employee,
        coalesce(s_admin.full_name, s_emp.full_name) as sender_name,
        r.full_name as recipient_name, r.email as recipient_email,
        (
          select coalesce(json_agg(json_build_object('name', re.full_name, 'read_at', mr.read_at) order by mr.read_at), '[]'::json)
          from message_reads mr join employees re on re.id = mr.employee_id
          where mr.message_id = msg.id
        ) as read_by
      from messages msg
      left join employees s_admin on s_admin.id = msg.sender_admin_id
      left join employees s_emp on s_emp.id = msg.sender_employee_id
...
-- Now also includes: any image attached, whether the message is one the
-- employee sent themself (so their own reports show up in their feed
-- too), and its id (needed so the frontend can mark it read).
...
as $$
  ...
  from (
    select
      msg.id, msg.body, msg.created_at, msg.image_data,
      (msg.recipient_employee_id is not null) as is_direct,
      (msg.sender_employee_id = p_employee_id) as is_mine,
      coalesce(s_admin.full_name, s_emp.full_name) as sender_name
    from messages msg
    left join employees s_admin on s_admin.id = msg.sender_admin_id
    left join employees s_emp on s_emp.id = msg.sender_employee_id
    join employees me on me.id = p_employee_id
    where msg.sender_employee_id = p_employee_id
       or msg.recipient_employee_id = p_employee_id
       or (msg.recipient_employee_id is null and msg.sender_admin_id is not null and msg.created_at >= me.created_at)
  ) m;
$$;

-- NEW: marks every message currently visible to this employee (that
-- isn't their own) as read by them — called once when they open the
-- Messages tab. Admin can then see who has actually seen each message.
create or replace function mark_all_messages_read(p_employee_id uuid)
returns json
language plpgsql
security definer
as $$
begin
  ...
  select msg.id, p_employee_id
  from messages msg
  join employees me on me.id = p_employee_id
  where msg.sender_employee_id is distinct from p_employee_id
    and (
      msg.recipient_employee_id = p_employee_id
      or (msg.recipient_employee_id is null and msg.sender_admin_id is not null and msg.created_at >= me.created_at)
    )
...
-- NEW: lightweight, frequently-pollable presence snapshot for the
-- Employees screen's live online/offline indicator — cheaper than
-- re-fetching the full employee list every few seconds.
...
grant execute on function admin_get_presence(uuid) to anon;
...
grant execute on function change_my_password(uuid, text, text) to anon;
grant execute on function admin_reset_employee_password(uuid, uuid, text) to anon;
...
grant execute on function admin_send_message(uuid, uuid, text, text) to anon;
grant execute on function employee_send_message(uuid, text, text) to anon;
grant execute on function mark_all_messages_read(uuid) to anon;
