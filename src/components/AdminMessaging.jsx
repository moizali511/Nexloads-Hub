...
export default function AdminMessaging({ adminId, employees }) {
  ...
  useEffect(() => {
    load()
    const interval = setInterval(load, 10000) // pick up employee-sent reports without a manual refresh
  ...
  return (
    <div className="card">
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginTop: 4 }}>
        Send updates to your team, and see reports (deal or payment screenshots, etc) employees send you here.
      </p>
      ...
      <div style={{ marginTop: 12, maxHeight: 480, overflowY: 'auto' }} className="scrollbar-thin">
          ...
          <div key={m.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border-soft)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                {m.is_from_employee
                  ? <span style={{ color: '#3ddc84', fontWeight: 600 }}>{m.sender_name} → Admin</span>
            ...
            {!m.is_from_employee && m.read_by && m.read_by.length > 0 && (
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 6 }} title={m.read_by.map((r) => `${r.name} — ${new Date(r.read_at).toLocaleString()}`).join('\n')}>
                ✓ Seen by {m.read_by.length} {m.read_by.length === 1 ? 'person' : 'people'}
              </div>
            )}
          </div>
        ))}
